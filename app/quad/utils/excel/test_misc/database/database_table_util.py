...
def tbl_rows_insert(database,tbl_name,tbl_col_name,tbl_rows):
    ...
    try:
        ...
        log.log(PRIORITY.FAILURE,msg="unable to insert ["+str(len(tbl_rows))+"] rows into table ["+tbl_name+"] in db [" + database.name +"] ["+e.message+"]")
        result=[-1]
    else:   
        log.log(PRIORITY.SUCCESS,msg="inserted ["+str(len(tbl_rows))+"] into table ["+tbl_name+"] in db [" + database.name +"]")
...
def tbl_query(database,exec_str):
    ...
    try:
        ...
        log.log(PRIORITY.FAILURE,msg="unable to execute query ["+str(exec_str)+"] in db [" + database.name +"] ["+e.message+"]")
        return([],[-1],exec_str) 
...
def tbl_to_xml(database,tbl_name,columns,whereclause=None,xmlroot=None):
    ...
    col_name,tbl_rows,_ = tbl_rows_get(database,tbl_name,columns,
                                       whereclause=whereclause)
    
    table_el = xmltree.SubElement(root,"table")
    table_el.set("name",tbl_name)
    for _tbl_row in tbl_rows:
        row_el = xmltree.SubElement(table_el,"row")
        for i in range(len(_tbl_row)):
            field_el = xmltree.SubElement(row_el,col_name[i])
            field_el.text = str(_tbl_row[i])
            
    print "added " + str(len(tbl_rows)) + " rows from table " + tbl_name
