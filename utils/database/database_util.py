...
class Database():
    ...
    def __init__(self,db_name,remove_flag=False,testexists=False):
        ''' testexists is True, then open will fail if sqlite db file does not exist
        otherwise it will create a new empty db file'''
        self.name = db_name
        self.remove_flag = remove_flag # remove database on exit (for testing)
        self.testexists = testexists
...
def tbl_list(database):
    '''purpose: list out all the tables in a database
       returns: list'''
    sql_str = "select tbl_name from sqlite_master "
    sql_str += "where type='table'"
...
def tbl_create(database,tbl_name,col_defn,tbl_pk_defn=[]):
    ...
    try:
        ...
        log.log(PRIORITY.FAILURE,msg="unable to create table ["+tbl_name+"] in db [" + database.name +"] ["+e.message+"]")
        raise e
    else:   
        log.log(PRIORITY.SUCCESS,msg="created table ["+tbl_name+"] in db [" + database.name +"]")
