...
def __getuserloadedmodules__():
    ...
    for builtinmod in python27builtin:
        try:
            ...
            log.log(PRIORITY.INFO,msg="info: could not pop builtin mod ["+builtinmod+"]")          
        
    for alwaysloaded in python27alwaysloaded:
        try:
            mods.remove(alwaysloaded)
        except:
            log.log(PRIORITY.INFO,msg="info: could not pop alwaysloaded mo ["+alwaysloaded+"]")   
...
def imported_sysclasses_remove(classes,thisfilename):
    ...
    _thisfilename =  splitext(basename(thisfilename))[0]
    
    for cls in list(classes):
        for usermod in usermodules:
            if hasattr(modules[usermod],cls) and usermod <> _thisfilename:
                log.log(PRIORITY.INFO,msg="remove cls ["+cls+"] from [" +",".join(classes) +"," + usermod+"]")
                try:
                    classes.remove(cls)
                except:
                    pass
