...
def capture_key_press():
    ...
    fields = ['char','keysym','keysym_num','type','delta']
    keypress = {}
    ...
    def key(event):
        ...
        _keypress = {}
        for field in fields:
            _keypress[field] = getattr(event,field)
    ...
    root.update()
    frame.event_generate('<Button-1>', x=0, y=0)
    frame.event_generate('<KeyRelease-s>')
    root.update()
...
def tkwidgetfactory(app,var,master,toplevel,**kwargs):
    ...
    class tkwidget(var.widgettype):
        ...
        def __init__(self,master,widgettype):
            ...
            #widgettype.__init__(self,master,var,**kwargs)
...
class TkImageLabelGrid(Frame):
    ...
    @logger(log)
    def _createwidget(self,master,gridname,
                      var,
                      width,height,
                      x,y,maxrows,maxcols,
    ...
    def select_next_value(self):        
        self.focus_get()._select_next_value()
    ...
    def modeset(self,event):
        ...
        elif event.keysym_num == 114:
            self.selection_clear()
            new_inputmode = "Normal"
        elif event.keysym_num == 82:
            self.clipboard_clear()
            new_inputmode = "Normal"
        elif event.keysym_num == 99:
            self.clipboard_add_selection()
        elif event.keysym_num == 105:
            self.select_next_value()
        elif event.keysym_num == 120:
    ...
    def refocus(self,event):
        ...
        if event.keycode==113 or event.keysym=='u' or event.keysym=='Left':
                x=x-1
        elif event.keycode == 114 or event.keysym=='o' or event.keysym=='Right':
                x=x+1
        elif event.keycode == 111 or event.keysym=='i' or event.keysym=='Up':
                y=y-1
        elif event.keycode == 116 or event.keysym=='j' or event.keysym=='Down':
                y=y+1
...
class TkGridCombobox(TkCombobox):
    ...
    def _select_next_value(self):
        ...
        current_value = self.sv.get()
        max_index = len(self.orig_values) - 1
        
        if current_value == "":
            current_index = -1
        else:
            current_index = self.orig_values.index(current_value)
        ...
        self.sv.set(self.orig_values[next_index])
